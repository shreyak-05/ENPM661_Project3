...
def input_start(prompt):
    ...
    """
    while True:
        print("Enter", prompt, "node (x between 5 and 1794, y between 5 and 494) (Sample Input: 10,10): ")
        input_str = input()
        A = [int(i) for i in input_str.split(',')]
        A_1 = (A[0], A[1])
        if not (0 <= A[0] < 1800 and 0 <= A[1] < 500):
...
def bfs_algorithm():
    ...
    while not Q.empty():
        ...
        for dx, dy in movements:
            ...
            if (0 <= new_x < 1800) and (0 <= new_y < 500) and check_obstacles((new_x, new_y)) and (new_x, new_y) not in visit:
                Q.put((new_x, new_y))
